...
**概念图解：与 LLM 后训练（RLHF）的对应**（帮助建立心智模型）

先纠正一个常见方向误解：**轨迹（trajectory）不是 NPU 产出的，是仿真产出
的**。NPU 只持有权重（策略网络），CPU 仿真持有物理世界；两端每 20ms 交换
一次——NPU 把动作（256 env × 29 个数）经共享内存发给仿真，仿真推进物理
并回传新观测 + reward，这些 (观测, 动作, reward) 序列攒成轨迹，凑满
32 步 × 256 env 后 NPU 才做 PPO 反向传播改权重。NPU 从头到尾不"拥有"轨迹，
它只是轨迹数据的消费者。

```
  LLM 后训练（RLHF）                     本文档的 PPO 微调
 ┌──────────────────────┐             ┌──────────────────────────┐
 │ policy = LLM          │             │ policy = Actor 网络（37M） │
 │ prompt → 生成回答      │             │ 观测 → 生成关节动作         │
 └──────┬────────▲──────┘             └──────┬──────────▲────────┘
     回答│      │reward                   动作│          │观测+reward
        ▼      │                              ▼          │
 ┌──────────────────────┐             ┌──────────────────────────┐
 │ reward model          │             │ MuJoCo 物理 + 打分函数     │
 │ （训练好的打分模型）    │             │ （物理定律 + 13 项追踪奖励）│
 └──────────────────────┘             └──────────────────────────┘
   凑一批回答 → PPO 更新 LLM 权重        凑 8192 条转移 → PPO 更新策略权重
```

| LLM 后训练 | 本文档链路 |
|---|---|
| 预训练基础模型 | 官方 `sonic_release/last.pt`（NVIDIA 在 Isaac、14 万段动捕上训好） |
| prompt | 当前观测 + 参考动捕未来 10 帧 |
| 回答（response） | 一条 rollout 轨迹 |
| reward model | 物理引擎 + 追踪奖励函数（裁判） |
| PPO 更新 | 同一个算法 |
| 后训练目的 | 基础模型 → 适配新域（这里是 MuJoCo 物理，不是新语言域） |

本质差别：LLM 的环境是静态文本分布，机器人的环境会**反作用力**——动作发
出去世界真的会变，所以语料（轨迹）必须靠仿真现造现用，这也就是这条链路
离不开仿真的原因。部署/评估（§3/§4）则相当于推理：权重冻结、只前向、不复盘。
