...
# Enable the URDF importer extension (required by UrdfFileCfg spawners, no version pin)
    ...
    for f in files:
        ...
        for mode in modes:
            ...
            for i, k in enumerate(range(args_cli.start, args_cli.end)):
                if _t0_probe is None:
                    _t0_probe = sim.current_time
                elif not _t_probe_emitted:
                    print(f"[probe] one ctrl step advances {sim.current_time - _t0_probe:.4f}s (expect 0.0200)", flush=True)
                    _t_probe_emitted = True
                ...
                # flush pending set_*_target buffers into physics
                robot.write_data_to_sim()
                # --- one control step = 4 physics substeps, target held ---
                for _ in range(DECIMATION):
                    sim.step()
                    robot.update(SIM_DT)  # refresh data buffers (no env.step here)
