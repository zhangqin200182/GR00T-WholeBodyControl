...
def main(override_config: omegaconf.OmegaConf):
    ...
    # ===== [R6] scene-2 (official env) runtime offsets + pinned 0mm tier =====
    import os as _os6
    import numpy as np
    sim = real_env.sim
    _os6.makedirs("/tmp/isaac_r6", exist_ok=True)
    _v = _robot.root_physx_view
    _co = _v.get_contact_offsets()[0].cpu().numpy()
    _ro = _v.get_rest_offsets()[0].cpu().numpy()
    _mp = _v.get_material_properties()[0].cpu().numpy()
    with open("/tmp/isaac_r6/offsets_scene2.txt", "w") as _f:
        _f.write("#### SCENE 2: official env (ModularTrackingEnvCfg via hydra) ####\n")
        _f.write(f"num_shapes={len(_co)}\n")
        _f.write("shape_idx | contact_offset | rest_offset | material(static,dyn,rest)\n")
        for _i in range(len(_co)):
            _f.write(f"{_i:3d} | {_co[_i]:.6f} | {_ro[_i]:.6f} | "
                     f"{_mp[_i][0]:.4f},{_mp[_i][1]:.4f},{_mp[_i][2]:.4f}\n")
        _f.write(f"contact_offset range: [{_co.min():.4f}, {_co.max():.4f}]  "
                 f"rest_offset all-zero: {bool((_ro == 0).all())}\n")
    print(f"[R6] scene2 offsets saved: co[{_co.min():.4f},{_co.max():.4f}] "
          f"ro_all_zero={bool((_ro == 0).all())}", flush=True)

    # ---- B: pinned tiers in the OFFICIAL env scene (settle scene's capsule
    #      feet never collide with ground there -- empirically proven; the
    #      official env's sensor DOES report foot forces, cf. P0/R3) ----
    _default_q = _robot.data.default_joint_pos.clone()
    _root_now = _robot.data.root_pos_w.clone()
    _pin_pos = _root_now.clone()
    _tier_z = float(os.environ.get("R6_PIN_Z", "0.7845"))
    _pin_pos[0, 2] = _tier_z
    _pin_quat = _robot.data.root_quat_w.clone()
    _pin_state = torch.cat([_pin_pos, _pin_quat,
                            torch.zeros(1, 6, device=_robot.device)], dim=1)
    _env_ids = torch.arange(1, device=_robot.device)
    _foot_names = [n for n in _robot.body_names
                   if "ankle_roll_link" in n or "ankle_pitch_link" in n]
    _foot_idx6 = {n: _robot.body_names.index(n) for n in _foot_names}
    # sensor body order may differ from robot body order
    _sb_names6 = list(_contact_sensor.body_names) if _contact_sensor else []
    _sidx6 = {}
    if _contact_sensor is not None:
        for n in _foot_names:
            if n in _sb_names6:
                _sidx6[n] = _sb_names6.index(n)
    _n6 = 100
    _rec6 = {"root_pos": [], "joint_q": [], "tau": []}
    for n6 in _foot_names:
        _rec6[f"pose_{n6}"] = []
        _rec6[f"force_{n6}"] = []
    print(f"[R6-B] pinned +0mm start (pin z=0.7845, reset-actual z={_root_now[0,2].item():.4f})", flush=True)
    for _s6 in range(_n6):
        _robot.set_joint_position_target(_default_q)
        _robot.write_root_state_to_sim(_pin_state, _env_ids)
        _robot.write_joint_state_to_sim(_default_q, torch.zeros_like(_default_q))
        _robot.write_data_to_sim()
        sim.step()
        _robot.update(0.005)
        if _contact_sensor is not None:
            _contact_sensor.update(0.02)
        _rec6["root_pos"].append(_robot.data.root_pos_w[0].cpu().numpy().copy())
        _rec6["joint_q"].append(_robot.data.joint_pos[0].cpu().numpy().copy())
        _rec6["tau"].append(_robot.data.applied_torque[0].cpu().numpy().copy())
        for n6 in _foot_names:
            _bi = _foot_idx6[n6]
            _rec6[f"pose_{n6}"].append(np.concatenate([
                _robot.data.body_pos_w[0, _bi].cpu().numpy(),
                _robot.data.body_quat_w[0, _bi].cpu().numpy()]))
            if _contact_sensor is not None and n6 in _sidx6:
                _rec6[f"force_{n6}"].append(
                    _contact_sensor.data.net_forces_w[0, _sidx6[n6]].cpu().numpy().copy())
            else:
                _rec6[f"force_{n6}"].append(np.zeros(3, np.float32))
    _out6 = {k: np.asarray(v) for k, v in _rec6.items()}
    np.savez(f"/tmp/isaac_r6/official_pinned_z{_tier_z:+.4f}.npz", **_out6)
    _t6 = slice(_n6 - 10, _n6)
    _lines6 = [f"official env pinned z={_tier_z} (100 steps, last-10-step means, joints locked)"]
    for n6 in _foot_names:
        _fm = np.linalg.norm(_out6[f"force_{n6}"][_t6], axis=1).mean()
        _pm = _out6[f"pose_{n6}"][_t6, :3].mean(axis=0)
        _lines6.append(f"  {n6}: pos=({_pm[0]:.4f},{_pm[1]:.4f},{_pm[2]:.4f}) |F|={_fm:8.2f}N")
    _lines6.append(f"  root_z tail={_out6['root_pos'][_t6, 2].mean():.4f} "
                   f"|tau| max={np.abs(_out6['tau'][_t6]).max():.2f}")
    _txt6 = "\n".join(_lines6)
    print(_txt6, flush=True)
    with open(f"/tmp/isaac_r6/official_pinned_z{_tier_z:+.4f}_summary.txt", "w") as _f:
        _f.write(_txt6 + "\n")
    import os as _os
    print("[R6] SCENE2_AB_DONE", flush=True)
    _os._exit(0)
    # ===== [/R6] =====
