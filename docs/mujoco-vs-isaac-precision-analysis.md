...
```
Euler + pyramidal + 手动参数:  21 步 (α≈0.013)
  + implicitfast:              25 步 (+16%)
  + cone=1 椭圆锥:              26-27 步 (+9%)
  ─────── 参数天花板 ───────    ~28 步 (α≈0.011)
  ─────── 引擎天花板 ───────    ≥35 步 (需要改 C++ 源码)
  ─────── Isaac 级别 ───────    100+ 步 (α<0.002)
```

**参数优化到此结束。** solref、damping、kp/kd、摩擦锥、积分器——所有 MuJoCo 允许我们动的参数都已确认最优或接近最优。28 步和 100 步之间 72 步的差距不是参数问题，是引擎架构问题。
