...
**初始状态说明**：我们侧起步用 `root_z_offset=0.04` 补偿动捕悬空，**起始相位固定 t=0**（从片段第 0 帧开始，初始 qpos = 第 0 帧参考位姿，初始速度为零）。你们侧同样从第 0 帧起步，按 Isaac 原生方式初始化，**记录初始 root 高度和初始 qpos 即可**（diff 时我们先对齐初始条件）。不要为对齐而改你们的环境逻辑——我们要的是 Isaac 原生行为。
...
| **起始相位** | **固定 t=0**（双方一致，写入 protocol_notes）| 我们侧 `--t0` 强制（2026-08-18 起）；此前随机相位因 12 片段均短于 max_ep（500 帧）已退化为 `uniform(0,0.001)`≈0，t=0 强制复测数字不变（release 25.67 / PD 34.33），附录 B 直接适用 |
...
## 附录 B：逐集对比表（PhysX 我们侧 + Isaac 后验判定）

> 0.35/0.35/1.5，sequential 配对，确定性，**起始相位 t=0**。数字为后验生存步数。
> **Isaac 侧判定说明**（数据有效性处理，详见训练日志）：①ref 轨迹航向相对化（Isaac 机器人从资产默认朝向起步、从不与 ref 航向对齐）；②wrist-h 检查剔除（上臂关节 mocap↔资产约定偏移 10-65°，策略从不跟踪 mocap 腕值；腿/踝链干净）；③grace=0 主数字（出生瞬态敏感性 87.33→94.67 @grace10，结论稳健）。
> **脚本自洽性**：`scripts/adjudicate_isaac_baseline.py` 在我们自己的 save npz 上复现 live 评测逐位一致（25.67 == 25.67）。

| ep | 片段 | 我们 release | 我们 PD | Isaac release | Isaac PD |
|----|------|-------------|---------|---------------|----------|
| 0 | walk_ff_loop_180_R_003__A050 | 28 | 28 | **8** | 28 |
| 1 | walk_the_dog_ff_180_loop_R_001__A476 | 26 | 23 | 27 | 13 |
| 2 | injured_R_leg_walk_ff_start_315_R_002__A232 | 29 | 57 | 11 | 14 |
| 3 | walk_sideway_045_loop_003__A033 | 34 | 42 | **322** | 34 |
| 4 | crutches_walk_arc_cw_start_R_001__A516 | 21 | 50 | **108** | 14 |
| 5 | walk_ff_stop_360_R_001__A418 | 19 | 16 | 11 | 14 |
| 6 | crutch_walk_turn_270_R_001__A518 | 20 | 27 | 2 | 38 |
| 7 | walk_ff_stop_270_002__A051_M | 23 | 21 | **313** | 14 |
| 8 | walk_into_door_R_001__A514 | 28 | 28 | 1 | 19 |
| 9 | inj_right_leg_walk_180_R_max_003__A078 | 25 | 36 | 62 | 27 |
| 10 | big_heavy_one_hand_walk_ff_start_360_R_001__A509 | 21 | 41 | **182** | 7 |
| 11 | injured_torso_walk_ff_start_225_R_003__A338 | 34 | 43 | 1 | 1 |
| **均值** | | **25.67** | **34.33** | **87.33** | **18.58** |

**裁决**：Isaac 侧 release 以 **4.70× 碾压 PD**（87.33 vs 18.58）——技能真实且强；我们侧 release 反输 PD（0.75×）。**我们环境压缩 release 技能 3.40×**，同时我们的超硬驱动增益把 PD 基线美化了 1.85×（34.33 vs 18.58）。差距最大的片段：walk_sideway（322 vs 34，9.5×）、walk_ff_stop_270（313 vs 23，13.6×）、big_heavy（182 vs 21，8.7×）——**这些正是轨迹级 diff ② 的最高信息量目标**。
