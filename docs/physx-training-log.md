...
## kick 协议首测 (08-20 深夜)：settle 缺陷修正后 I_pivot 首读 ≈ 18 kg·m²

- 第一版 settle 把所有关节 e=0 跟踪（无任何恢复力矩）→ settle 本身自由发散（k=0 已 −29°）。修正：只踝 e=0（枢轴无恢复力矩）、髋/膝固定持姿 → 均衡位保持 −0.4° ✓。
- 踢击响应结构：前倾速率 −0.03 rad/s（方向正确）→ 姿态驱动 6 子步后反转（0.008→0.06 增长）→ **1.0s 掀翻**（与 1.2s 时间常数一致）。
- I_pivot 首读 17.8 kg·m²（m·v0·h/θ̇(0+)；H_PIVOT=0.75 粗估，待审核方解析 I_pivot 校准）。λ 拟合被驱动反转段污染（λ=48 无效）——下一轮：窗口细化（踢击后 0-25ms 纯发散段）+ 解析 I_pivot 锚定。
- 方向性确认（审核方更正采纳）：我们抗性更弱（更快翻）——"过刚"旧框架撤回；统一图景 = 躯干质量没有动态拖拽身体。

## kick v4 + Isaac 力矩取证 (08-21)：枢轴之谜破解——他们的脚会滚，我们的脚钉死

**审核方校准落地（本地复算验证）**：m_total=34.392（MuJoCo ✓，逐位）、I_pivot 踝线=20.47（FK 20.465 ✓ 复算 20.47 差 0.7%）、I_CoM 侧向=3.045、mgh=240.1。compute_meff.py 父系约定退化已修（0601af6 引入，踝 M_eff 虚高 38×）：修复后踝 0.003/膝 0.116 与部署表逐位 ✓；臂条目（elbow 0.082 vs 部署 0.034）provenance 未明，臂增益重导前须查清。XML 4 处惯量修正已与服务器逐位一致（md5 相同）。

**kick v4（全持力，FK@实际 q，冲量质量修正）推翻近临界读法**：我们植物**深度不稳定**——速率 0.16→2.58 rad/s（0.12-1.0s 干净指数）→ λ≈3.2/s → kp_eff ≈ 242.6−10.2·20.5 ≈ **34 Nm/rad ≈ 踝驱动 28.5 + 接触 ~5**。近临界框架（Δ≈14、kp_eff≈224）在我们侧被证伪——1.2s 非时间常数，是漂移期+崩塌的组合计时。

**Isaac 侧同口径重算**：他们 pitch(t) 2s 内 −0.7°→+12.2°，θ̈≈0.14-0.24 rad/s² 随 θ 增长 → kp_eff ≈ **220-229**（近临界 ✓ 审核方 ≥234 读法方向正确）。差距 ≈ **190 Nm/rad**。

**🔴 决胜证据（他们 npz 的 applied_torque）**：他们体倾 12.6° 期间，**踝力矩恒定**（τ_la +1.3→+2.4、τ_ra −20 恒定），踝关节只动 1.1°——**他们的脚跟着身体滚**（趾缘枢轴，接触柔顺）；我们踝关节张开 0.72 rad（e=28.5·θ 需 0.85 rad 才能平衡）——**脚钉死平贴**（刚性多点接触），身体绕踝线崩。**枢轴不同**：他们=趾缘滚动（dF/dz −1204 弹簧式 ✓ 一致），我们=踝线（+980 反弹簧 ✓ 一致）。驱动逐位相同（他们的 τ vs e 斜率 = 28.5 ✓），差异 100% 在脚-地界面柔顺度。

**box vs 风扇脚 A/B**：逐位相同崩塌（settle 高度 0.7535→0.7333 证明风扇已生效）→ 几何不是机制，**接触柔顺度才是**。e8_launch 里 SONIC_PHYSX_BOUNCE_THRESHOLD / FRICTION_OFFSET / FRICTION_CORR_DIST 三个 Isaac 接触参数**无人读取（死变量）**——这是当年的接线 TODO，很可能就是柔顺接触的实现口。
