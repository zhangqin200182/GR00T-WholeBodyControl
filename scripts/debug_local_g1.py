"""Test G1 with local poses + real CoM: FK + physics + ref PD."""
...
print("\n=== Test 1: FK (local poses + real CoM) ===")
px.init_foundation()
...
print(f"  Max FK error: {max_err:.6f}m at body {max_body} '{fk.TRACKED_BODIES[max_body]}'")
print(f"  {'✅ FK correct!' if max_err < 0.01 else '❌ FK wrong'}")

# Check if bodies are at correct world position after createLink
for i in range(3):
    ap = art.get_link_world_pose(i)[0]
    tp = tracked[i][0]
    print(f"  body {i} ({fk.TRACKED_BODIES[i]}): PhysX={ap} FK={tp} err={np.linalg.norm(np.array(ap)-tp):.4f}")
px.release_foundation()

# ── Test 2: Zero-G stability ──
print("\n=== Test 2: Zero-G stability (no PD) ===")
px.init_foundation()
...
for step in range(20):
    ...
    if step < 3 or step % 5 == 0:
        print(f"  step {step}: root_z={rp[2]:.4f} q_range=[{jp.min():.4f},{jp.max():.4f}]")
    scene2.simulate(0.002); scene2.fetch_results()
px.release_foundation()

# ── Test 3: Zero-G + PD hold ──
print("\n=== Test 3: Zero-G + PD hold ===")
px.init_foundation()
...
art3.set_root_world_pose(ref_pos.astype(np.float32), np.array([1,0,0,0], dtype=np.float32))
...
# ── Test 4: Gravity + PD hold ──
print("\n=== Test 4: Gravity + ground + PD hold ===")
px.init_foundation()
...
for step in range(30):
    ...
    if step < 3 or step % 5 == 0:
        je = np.linalg.norm(jp - ref_qpos)
        print(f"  step {step}: root=({rp[0]:.3f},{rp[1]:.3f},{rp[2]:.3f}) jerr={je:.4f}")
    scene4.simulate(0.002); scene4.fetch_results()
px.release_foundation()

# ── Test 5: Motion tracking ──
print("\n=== Test 5: Motion tracking 20 frames ===")
px.init_foundation()
art5 = physx_loader.load_g1(px, XML)
...
idx0 = 500
art5.set_root_world_pose(pk["root_trans_offset"][idx0].astype(np.float32), np.array([1,0,0,0], dtype=np.float32))
art5.set_joint_positions(pk["dof"][idx0].astype(np.float32))
art5.set_joint_velocities(np.zeros(29, dtype=np.float32))

for step in range(20):
    ref = pk["dof"][idx0+step].astype(np.float32)
    art5.set_joint_drive_targets(ref)
    rp = art5.get_root_world_pose()[0]; jp = art5.get_joint_positions()
    if np.any(np.isnan(jp)) or np.any(np.isnan(rp)):
        print(f"  step {step}: NaN!"); break
    je = np.linalg.norm(jp - ref)
    print(f"  step {step}: root_z={rp[2]:.4f} jerr={je:.4f}")
    scene5.simulate(0.002); scene5.fetch_results()
