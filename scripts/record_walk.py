#!/usr/bin/env python3
"""Record the SONIC policy walking in MuJoCo to an mp4 (honest mode by default).

Follows the robot with the camera; renders one frame per control step (50 fps).
Usage: python scripts/record_walk.py [n_steps] [out.mp4]

Honest mode (default): the policy controls all 29 joints; the reference motion
advances at its native fixed rate (exactly like training). Optional legacy hacks
from the zero-shot era are opt-in via env vars:
  SONIC_REC_UPPERBODY=1  kinematic upper-body override from the reference
  SONIC_REC_PLL=1        warp reference timeline to follow the robot
  SONIC_REC_NO_TERM=1    ignore terminations (long-run mode); default stops at
                         the first termination and reports the step count.
"""
import os
import sys
...
out = sys.argv[2] if len(sys.argv) > 2 else "/Users/max/code/ai/embody/g1_walk.mp4"
upperbody = os.environ.get("SONIC_REC_UPPERBODY") == "1"
pll = os.environ.get("SONIC_REC_PLL") == "1"
no_term = os.environ.get("SONIC_REC_NO_TERM") == "1"
motion_idx = os.environ.get("SONIC_MOTION_IDX")          # fix motion selection
stochastic = os.environ.get("SONIC_STOCH") == "1"        # sample actions (as training)
novideo = os.environ.get("SONIC_NOVIDEO") == "1"         # stats only, no frames
...
if motion_idx is not None:
    ...
    def _sample_motion_fixed(e=env, sel=_sel):
        m = e.motions[sel]
        dof = m["dof"]; n = len(dof)
        e._ref_dof = dof
        e._ref_root_rot = m["root_rot"].astype(np.float64)
        e._ref_root_trans = m["root_trans_offset"].astype(np.float64)
        e._ref_fps = m.get("fps", 30.0)
        e._ref_dt = 1.0 / e._ref_fps
        max_time = (n - e.max_ep - 1) * e._ref_dt
        e._ref_time = float(np.random.uniform(0, max(0.001, max_time)))

    env._sample_motion = _sample_motion_fixed
obs = env.reset()
path = 0.0
...
for step in range(n_steps):
    ...
    if os.environ.get("SONIC_NOCLIP") != "1":
        a = np.clip(a, -1.0, 1.0)
    if upperbody:
        ref = env._ref_dof[min(int(env._ref_time * env._ref_fps), len(env._ref_dof) - 1)]
        a[12:] = np.clip((ref[12:] - env.act_offset[12:]) / env.act_scale[12:], -1.0, 1.0)
    ...
    if pll:
        ...
        ref_p = env._ref_root_pos()
        ref_v = env._ref_root_trans[nxt_idx][:2] - ref_p[:2]
        speed = np.linalg.norm(ref_v)
        if speed > 1e-4:
            along = np.dot(xy - ref_p[:2], ref_v / speed)
            env._ref_time = float(
                np.clip(env._ref_time + np.clip(along, -0.5, 0.5) * 0.1, 0.0, motion_len - 1e-3)
            )
